...
# Update gammu config with the correct serial port
sed -i "s|device = .*|device = $SERIAL_PORT|" /etc/gammu-smsdrc
...
send_queued_sms() {
    if [ -s "$SMS_QUEUE" ]; then
        ...
        if [ -n "$number" ] && [ -n "$message" ]; then
            ...
            local result
            result=$(echo "$message" | gammu --config /etc/gammu-smsdrc sendsms TEXT "$number" 2>&1)
            local exit_code=$?
